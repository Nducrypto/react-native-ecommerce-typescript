...
import { mergeSort } from "../actions/utilityFunctions";
...
export const customerSpecificChat = (customerId: string) => {
  const { allChat } = useAllChatState();
  const messages = allChat[customerId] || [];
  return messages;
};

export const messageNotificationForCustomer = (customerId: string) => {
  const messages = customerSpecificChat(customerId);
